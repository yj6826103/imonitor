<template>
  <div class="uploadArticle" v-title="'上传文章'">
    <!--顶部-->
    <iM_Topbar></iM_Topbar>
    <!--头部-->
      ...
      <div id="content" :class="showSiderbar?'showSiderbar':'hiddenSiderbar'"
        ...
        <div :style="'width:'+contentWidth+'px;height:calc(100% - 5px);'">
          ...
          <div id="content_page" :class="showMenu?'showMenu':'hiddenMenu'">
            <!--此处放置页面-->
            <UploadArticleContent></UploadArticleContent>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  import UploadArticleContent from './UploadArticleContent';

  export default {
    name: "add-article",
    components: {iM_Topbar, iM_Header, iM_Sidebar, iM_Menu, UploadArticleContent,},
    data() {
      return {
        showSiderbar: false,
        showMenu: false,
